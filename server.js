...
app.get("/api/mindFoodRestaurants", async (req, res) => {
  try {
    const { itemId, item,lat,lng } = req.query;
    console.log(itemId, item, lng);
    console.log(lng);
    const url =
      `https://www.swiggy.com/dapi/restaurants/list/v5?lat=${lat}&lng=${lng}&collection=${Number(itemId)}&tags=layout_CCS_${item}&sortBy=&filters=&type=rcv2&offset=0&page_type=null`;
